...
<p>よって、練習問題3の時のような配列を作れば良いです。<br>
具体的には、`memo[i] = 整数iをつくるための操作の最小回数`のような配列を作り、作れた整数が、操作の最小回数を更新した時に限って、その後の操作を続ければ良いです。</p>
<p>このような処理によって、2種類以上の作り方が存在する整数について<br>
(ex. `1->2->4->3` or `1->2->3`)
も、正しく操作の最小回数を求めることができます。
</p>
<p>"最小回数を更新する"回数は、\(K \leq 20\)</p>の時さほど多くないことから、十分高速です。</p>
ソースコード↓
